<template>
  <div class="drugApplyW">
    <el-form label-width="120px" :model="applytable" :rules="rules" ref="applyform" class="apply-form">
      <h4 class="applytable"  style="margin-bottom:50px">药物补给申请表</h4>
      ...
      <el-form-item label="申请时间" prop="date">
        <el-date-picker v-model="applytable.date" type="date" placeholder="选择日期" style="width:380px"></el-date-picker>
      </el-form-item>
      <p  class="submit">
  ...
  export default {
    ...
    methods: {
      ...
      getinsufficientData: function () {
        ...
        drugapplyThis.$http.get('/api/drug/list').then(function(res) {
          // 把json接口获取的数据
       
          for (let i = 0; i < res.body.drugs.length; i++) {
            if (res.body.drugs[i].inventory < 10) {
              drugapplyThis.insufficientTable.push(res.body.drugs[i]);
            }
          }
        }, res => {
          // error callback
